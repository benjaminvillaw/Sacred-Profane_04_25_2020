...
income <- read_excel("Data/income.xlsx", sheet = NULL, range = NULL, col_names = TRUE,
  col_types = NULL, na = c("**","*","#"), trim_ws = TRUE, skip = 0,
  n_max = Inf, guess_max = min(1000, 2000),
  progress = readxl_progress(), .name_repair = "unique")
...
```{r Gallup Data Read In}

socialwell_index <-read_excel("Data/social_wellbeing_index.xlsx", sheet = NULL, range = NULL, col_names = TRUE,
  col_types = NULL, na = c("**","*","#"), trim_ws = TRUE, skip = 0,
  n_max = Inf, guess_max = min(1000, 2000),
  progress = readxl_progress(), .name_repair = "unique")

social_hours <-read_excel("Data/hours_spent_socially.xlsx", sheet = NULL, range = NULL, col_names = TRUE,
  col_types = NULL, na = c("**","*","#"), trim_ws = TRUE, skip = 0,
  n_max = Inf, guess_max = min(1000, 2000),
  progress = readxl_progress(), .name_repair = "unique")

religion_important <-read_excel("Data/religion_important_GallupAnalytics.xlsx", sheet = NULL, range = NULL, col_names = TRUE,
  col_types = NULL, na = c("**","*","#"), trim_ws = TRUE, skip = 0,
  n_max = Inf, guess_max = min(1000, 2000),
  progress = readxl_progress(), .name_repair = "unique")

church_frequency <- read_excel("Data/church_frequency.xlsx", sheet = NULL, range = NULL, col_names = TRUE,
  col_types = NULL, na = c("**","*","#"), trim_ws = TRUE, skip = 0,
  n_max = Inf, guess_max = min(1000, 2000),
  progress = readxl_progress(), .name_repair = "unique")
...
socialwell_index <- socialwell_index %>%
  clean_names() %>%
  filter(time==2016) %>%
...
church_frequency <- church_frequency %>%
 clean_names() %>%
  filter(time==2010) %>%
...
depression2 <- depression %>%
  ...
  rename(area_title = geography) %>%
  arrange(desc(yes)) %>%
  select(area_title,yes)

depression2 %>%
ggplot(aes(yes)) + geom_histogram()


joined_gallup


test <- left_join(joined_gallup
,depression2, by = "area_title")


test %>%
 ggplot(aes(church_week,yes))+ geom_point() + geom_smooth(method = "lm", formula = y ~ x, se = FALSE) +
                scale_y_log10() + scale_x_log10()+
                theme_classic()

test %>%
  select(church_week,yes,social_hours,religion_imp,social_index) %>%
cor()
...
dep_religion <- inner_join(jobs_religion, depression2, by= "area_title")
write_rds(dep_religion,"shiny/dep_religion.rds")


model <- lm(jobs_1000_orig~yes, data = dep_religion)
